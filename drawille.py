...
def get_pos(x, y):
    ...
    return normalize(x) // X_SIZE_RATIO, normalize(y) // Y_SIZE_RATIO 
...
class Canvas(object):
    ...
    def set(self, x, y):
        ...
        self.chars[row][col] |= pixel_map[y % Y_SIZE_RATIO][x % X_SIZE_RATIO]
    ...
    def toggle(self, x, y):
        ...
        if type(self.chars[row][col]) != int or self.chars[row][col] &\
           pixel_map[y % Y_SIZE_RATIO][x % X_SIZE_RATIO]:
            self.unset(x, y)
        else:
            self.set(x, y)
    ...
    def get(self, x, y):
        ...
        x = normalize(x)
        y = normalize(y)
        dot_index = pixel_map[y % Y_SIZE_RATIO][x % X_SIZE_RATIO]
        col, row = get_pos(x, y)
